...
public class AccountControllerTest {
    ...
    public void shouldGetAccountByName() throws Exception {
        ...
        mockMvc.perform(get("/"
                                + account.getName())).andExpect(jsonPath("$.name").value(account.getName())).andExpect(status().isOk());
    }
    ...
    public void shouldSaveCurrentAccount() throws Exception {
        ...
        mockMvc.perform(put("/current").principal(new UserPrincipal(account.getName())).contentType(MediaType.APPLICATION_JSON).content(json)).andExpect(status().isOk());
    }
    ...
    public void shouldFailOnValidationTryingToSaveCurrentAccount() throws Exception {
        ...
        mockMvc.perform(put("/current").principal(new UserPrincipal(account.getName())).contentType(MediaType.APPLICATION_JSON).content(json)).andExpect(status().isBadRequest());
    }
    ...
    public void shouldRegisterNewAccount() throws Exception {
        ...
        mockMvc.perform(post("/").principal(new UserPrincipal("test")).contentType(MediaType.APPLICATION_JSON).content(json)).andExpect(status().isOk());
    }
    ...
    public void shouldFailOnValidationTryingToRegisterNewAccount() throws Exception {
        ...
        mockMvc.perform(post("/").principal(new UserPrincipal("test")).contentType(MediaType.APPLICATION_JSON).content(json)).andExpect(status().isBadRequest());
    }
}
