var user = {}, savings = {}, incomes = {}, expenses = {};
...
function initAccount(account) {
  ...
  if (account.incomes) {
    for (i = 0; i < account.incomes.length; i++) {
      AddIncome(i
          + 1, account.incomes[i].title, account.incomes[i].icon, account.incomes[i].currency, account.incomes[i].period, account.incomes[i].amount);
    }
  }
  ...
  if (account.expenses) {
    for (j = 0; j < account.expenses.length; j++) {
      AddExpense(j
          + 1, account.expenses[j].title, account.expenses[j].icon, account.expenses[j].currency, account.expenses[j].period, account.expenses[j].amount);
    }
  }
}
...
function AddIncome(income_id, title, icon, currency, period, amount) {
  incomes[income_id] = {
    income_id: income_id, title: title, icon: icon, currency: currency, period: period, amount: amount.toString()
  }
}

function AddExpense(expense_id, title, icon, currency, period, amount) {
  expenses[expense_id] = {
    expense_id: expense_id, title: title, icon: icon, currency: currency, period: period, amount: amount.toString()
  }
}
...
function initGreetingPage() {
  ...
  initStatisticPage();
  setTimeout(function () {
    initSavingsCircles(user.checkedPercent, 0.2, savings.freeMoney, savings.freeMoney, 700)
  }, 200);
  initSavingsSlider();
...
function initSettingsPage() {
  switch (user.checkedCurr) {
    case "RUB":
      $("#rublesign").css({"background-position": "-150px 0"});
      break;
    case "EUR":
      $("#rublesign").css({"background-position": "-386px 0"});
      break;
    case "USD":
      $("#rublesign").css({"background-position": "-354px 0"});
      break;
  }
...
function addSavings() {
  ...
  switch (savings.capitalization) {
    case true:
      $("#capitalization").prop('checked', true);
      break;
  }
...
function addItems() {
  Object.keys(incomes).forEach(function (key) {
    ...
    $("#incomeslider").append('<div onclick="itemClick(this)" class="incomeitem" id="income-'
        + incomes[key].income_id
        + '"><span class="title11museo300">'
        + incomes[key].title
        + '</span><p class="title9museo300"><span class="bolddigit20">'
        + value
        + ' </span>'
        + checkCurrency(incomes[key].currency)
        + checkPeriod(incomes[key].period)
        + '</span></p><div class="itembackground"></div></div>');
    $("#income-" + incomes[key].income_id).data({
      "id": incomes[key].income_id,
      "icon": incomes[key].icon,
      "amount": incomes[key].amount,
      "title": incomes[key].title,
      "currency": incomes[key].currency,
      "period": incomes[key].period
    }).children("div").addClass(incomes[key].icon);
  });
  Object.keys(expenses).forEach(function (key) {
    ...
    $("#expenseslider").append('<div onclick="itemClick(this)" class="expenseitem" id="expense-'
        + expenses[key].expense_id
        + '"><span class="title11museo300">'
        + expenses[key].title
        + '</span><p class="title9museo300"><span class="bolddigit20">'
        + value
        + ' </span>'
        + checkCurrency(expenses[key].currency)
        + checkPeriod(expenses[key].period)
        + '</span></p><div class="itembackground"></div></div>');
    $("#expense-" + expenses[key].expense_id).data({
      "id": expenses[key].expense_id,
      "icon": expenses[key].icon,
      "amount": expenses[key].amount,
      "title": expenses[key].title,
      "currency": expenses[key].currency,
      "period": expenses[key].period
    }).children("div").addClass(expenses[key].icon);
  });
...
function Up(transaction) {
  ...
  var itemWidth = 71;
  if (Math.abs($("#" + transaction + "slider").position().top % itemWidth) > 0.01) {
    return;
  } else if ($("#"
          + transaction
          + "slider").position().top > -(length - 4) * itemWidth) {
...
// ADD ITEMS
function addNewItem() {
  var itemTitle = $(".modaltitle").val(), itemIcon = $(".initicons").data("iconselected"),
      itemCurrency = $(".modalcurrency").val(), itemPeriod = $(".modalperiod").val(),
      itemValue = $(".modalvalue").autoNumeric("get"), itemId = 0;
  if (checkModalFields(itemValue, itemTitle)) { // If inputs are proper filled, add incomes or expenses
    if ($(".initicons").data("incomes-expenses") == "incomes") {
      Object.keys(incomes).forEach(function (keys) {
        if (incomes[keys].income_id > itemId) {
          itemId = (incomes[keys].income_id)
        }
      });
      ...
      Object.keys(expenses).forEach(function (keys) {
        if (expenses[keys].expense_id > itemId) {
          itemId = (expenses[keys].expense_id)
        }
      });
...
function addNewDiv(whichColumn, itemId, itemTitle, itemIcon, itemCurrency, itemPeriod, itemValue) {
  ...
  $("#" + whichColumn + "slider").append('<div onclick="itemClick(this)" class="'
      + whichColumn
      + 'item" id="'
      + whichColumn
      + '-'
      ...
      + value
      + ' </span>'
      + checkCurrency(itemCurrency)
      + checkPeriod(itemPeriod)
      + '</span></p><div class="itembackground"></div></div>');
  $("#" + whichColumn + "-" + itemId).addClass("newitemadded").data({
    "id": itemId,
    "icon": itemIcon,
    "amount": itemValue,
    "title": itemTitle,
    ...
    "period": itemPeriod
  }).children("div").addClass(itemIcon);
  setTimeout(function () {
    $("#" + whichColumn + "-" + itemId).removeClass("newitemadded")
  }, 4100);
}
// EDIT ITEMS
function itemClick(item) {
  // Add delete-button
  $(".modal-delete").show();
  var itemDiv = $("#" + item.id), itemIcon = itemDiv.data("icon"), itemCurrency = itemDiv.data("currency"),
      itemPeriod = itemDiv.data("period"), itemValue = itemDiv.data("amount"), itemTitle = itemDiv.data("title"),
      itemId = itemDiv.data("id"), incomesExpenses = "expense", whichColumn = expenses;
...
function editOldDiv(whichColumn, itemId, itemTitle, itemIcon, itemCurrency, itemPeriod, itemValue) {
  var value = itemValue.replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 </span><span class="lightdigit20">');
  $("#" + whichColumn + "-" + itemId).replaceWith('<div onclick="itemClick(this)" class="'
      + whichColumn
      + 'item" id="'
      + whichColumn
      + '-'
...
function turnOffModal() {
  ...
  setTimeout(function () {
    ...
    $(".modaltitle").val('').hide();
    $(".modal-delete").hide();
    $("textarea#notes").val(user.notes).hide();
  }, 200);
}
...
$("#savingsvalue").bind("keyup keydown keypress select click", function () {
  savings.freeMoney = $("#savingsvalue").autoNumeric("get");
  moveRuble();
}).keyup(function (e) {
  if (e.which == 13) {
    this.blur()
...
function launchStatistic() {
  if ($("#incomeslider").children().length > 0 && $("#expenseslider").children().length > 0) {
    ...
    setTimeout(function () {
      $(".toppage, .bottompage").addClass("sectionDown");
    }, 400);
...
function jsonDataSave() {
  if (global.savePermit) {
    $.ajax({
      ...
      data: JSON.stringify({
        note: user.notes, incomes: $.map(incomes, function (value) {
          return [value]
...
function fadeStatistic() {
  ...
  setTimeout(function () {
    $("#lastlogoflipper").removeClass("flippedcard");
  }, 220);

  setTimeout(function () {
    drawChartLine(91);
    $(".bottompage").css({"display": "none"});
  }, 500);
}