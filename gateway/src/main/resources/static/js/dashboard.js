...
$("#inner-circle").knob({
  "readOnly": true, "width": 223, "height": 223, "thickness": 0.1, "displayInput": false, "fgColor": "#e6eff1"
});

$("#outer-circle").data({"width": 265}).knob({
  "readOnly": true, "width": 265, "height": 265, "thickness": 0.13, "displayInput": false, "fgColor": "#a59b9e"
});
...
$("#first-circle").data({"width": 165}).knob({
  "readOnly": true, "width": 165, "height": 165, "thickness": 0.14, "displayInput": false, "fgColor": "#efefef"
});

$("#second-circle").data({"width": 165}).knob({
  "readOnly": true, "width": 165, "height": 165, "thickness": 0.14, "displayInput": false, "fgColor": "#e0ded5"
});

$("#third-circle").data({"width": 165}).knob({
  "readOnly": true, "width": 165, "height": 165, "thickness": 0.14, "displayInput": false, "fgColor": "#b6aeb0"
});
...
function getConverted(column) {
  ...
  for (var key in column) {
    switch (column[key].currency) {
      case "RUB":
        column[key].converted = column[key].amount;
        break;
      case "EUR":
        column[key].converted = (column[key].amount * global.eur).toFixed(3);
        break;
      case "USD":
        column[key].converted = (column[key].amount * global.usd).toFixed(3);
        break;
    }
    switch (column[key].period) {
      case "MONTH":
        break;
      case "HOUR":
        column[key].converted = (column[key].converted * 730).toFixed(3);
        break;
      case "DAY":
        column[key].converted = (column[key].converted * 30.41666667).toFixed(3);
        break;
      case "QUARTER":
        column[key].converted = (column[key].converted / 3).toFixed(3);
        break;
      case "YEAR":
        column[key].converted = (column[key].converted / 12).toFixed(3);
        break;
    }
    switch (user.checkedCurr) {
      case "RUB":
        break;
      case "EUR":
        column[key].converted = (column[key].converted / global.eur).toFixed(3);
        break;
      case "USD":
        column[key].converted = (column[key].converted / global.usd).toFixed(3);
        break;
    }
    if (column == incomes) {
      if (firstitem == undefined) {
        firstitem = key;
        $("#incomeslider").data("firstitem", key);
      }
      incomesSumMonth += Math.round(column[key].converted);
      $("#circle-select-1, #circle-select-2, #circle-select-3").append('<option value="'
          + parseInt(column[key].income_id, 10)
          + '">'
          + escape(column[key].title)
          + '</option>');
    } else {
      if (firstitem == undefined) {
        firstitem = key;
        $("#expenseslider").data("firstitem", key);
      } else if (seconditem == undefined) {
        seconditem = key;
        $("#expenseslider").data("seconditem", key);
      }
      expensesSumMonth += Math.round(column[key].converted);
      $("#circle-select-1, #circle-select-2, #circle-select-3").append('<option value="'
          + (parseInt(column[key].expense_id, 10) + 100)
          + '">'
          + escape(column[key].title)
          + '</option>');
    }
...
function initStatisticPage() {
  ...
  changeCurrency = function () {
    switch (user.checkedCurr) {
      case "RUB":
        if (user.lastCurr == "RUB") {
          break;
        } else if (user.lastCurr == "USD") {
          savings.freeMoney = (savings.freeMoney * global.usd).toFixed(3);
        } else if (user.lastCurr == "EUR") {
          savings.freeMoney = (savings.freeMoney * global.eur).toFixed(3);
        }
        break;
      case "EUR":
        if (user.lastCurr == "EUR") {
          break;
        } else if (user.lastCurr == "USD") {
          savings.freeMoney = (savings.freeMoney * global.usd / global.eur).toFixed(3);
        } else if (user.lastCurr == "RUB") {
          savings.freeMoney = (savings.freeMoney / global.eur).toFixed(3);
        }
        break;
      case "USD":
        if (user.lastCurr == "USD") {
          break;
        } else if (user.lastCurr == "EUR") {
          savings.freeMoney = (savings.freeMoney * global.eur / global.usd).toFixed(3);
        } else if (user.lastCurr == "RUB") {
          savings.freeMoney = (savings.freeMoney / global.usd).toFixed(3);
        }
        break;
  ...
  initCircle = function (whichcircle, item, beforevalue) {
    ...
    switch (whichcircle) {
      case "first":
        ...
        break;
      case "second":
        animatecircle.call($("#second-circle"), beforevalue, (item.converted / 30.41666667).toFixed(1), (sum
        / 30.41666667), item.title);
        circle = 2;
        $("#second-circle").data({"item": item, "sum": sum});
        break;
      case "third":
        animatecircle.call($("#third-circle"), beforevalue, Math.round(item.converted * 1.2) * 10, (sum * 1.2)
            * 10, item.title);
        circle = 3;
    ...
    setTimeout(function () {
      $("#circle-select-" + circle + " option[value=" + value + "]").attr('selected', 'selected');
      $('select').trigger('refresh');
    }, 100)
    $("#circle-select-" + circle + "-back").removeClass().addClass(item.icon);
  }

  if (incomes[$("#incomeslider").data("firstitem")]
      != undefined
      && expenses[$("#expenseslider").data("firstitem")]
      != undefined) {
    initCircle("first", incomes[$("#incomeslider").data("firstitem")], 0);
    initCircle("second", expenses[$("#expenseslider").data("firstitem")], 0);
    if ($("#expenseslider").data("seconditem") !== undefined) {
      initCircle("third", expenses[$("#expenseslider").data("seconditem")], 0);
    } else {
      initCircle("third", expenses[$("#expenseslider").data("firstitem")], 0);
    }
  }

  $("#expenses-lines-container").html('<div class="lines-title"> Expenses structure <span class="grey">('
      + separateNumber(expensesSumMonth)
      + '<span class="greysmall"><span class="greysmallval curr"></span>/Month</span>)<span></span></div>')
  $("#incomes-lines-container").html('<div class="lines-title"> Incomes structure <span class="grey">('
      + separateNumber(incomesSumMonth)
      + '<span class="greysmall"><span class="greysmallval curr"></span>/Month</span>)<span></span></div>')

  var incomesIdSorted = Object.keys(incomes).sort(function (a, b) {
    return incomes[b].converted - incomes[a].converted
  });
  var expensesIdSorted = Object.keys(expenses).sort(function (a, b) {
    return expenses[b].converted - expenses[a].converted
  });

  initlines = function (column) {
    var container, idSorted, maxConvertedId, i;
    if (column == incomes) {
      container = "#incomes-lines-container";
      idSorted = incomesIdSorted;
      maxConvertedId = incomesIdSorted[0];
      sum = incomesSumMonth;
      i = 0
    } else {
      container = "#expenses-lines-container";
      idSorted = expensesIdSorted;
      maxConvertedId = expensesIdSorted[0];
      sum = expensesSumMonth;
      i = 100
    }

    idSorted.forEach(function (id) {
      $(container).append('<div id="line-'
          + i
          + '" class="itemline"><span class="lineitemtitle lightcircletitle">'
          + column[id].title
          + '</span><div class="lineitempercent">'
          + Math.round(100 * column[id].converted / sum)
          + '%</div><div class="lineitemvalue">'
          + separateNumber(Math.round(column[id].converted))
          + ' <span class="boldcircletitle curr lineitemcurr"></span><span class="lightcircletitle lineitemcurr">/Month</span></div><div class="leftpoint"></div><div id="linebackground-'
          + i
          + '" class="itemlinebackground"></div></div>');
      $("#line-" + i).data({"item": column[id]}).css({
        "width": Math.round(100
            * column[id].converted
            / column[maxConvertedId].converted) + "%"
      });
  ...
  initSavingsCircles = function (slidervalue, lastslidervalue, beforevalue, beforemiddlevalue, animatetime) {
    ...
    var monthSavings = [], depositMonthSavings = [], delta = incomesSumMonth - expensesSumMonth,
        deltaDeposit = delta * slidervalue, deltaNoDeposit = delta * (1 - slidervalue), percent = savings.percent,
        deltapercents = 0;
    ...
    if (delta >= 0) {	// if incomes > expenses
      if (savings.deposit) {	// deposit turned on
        if (savings.capitalization) { // capitalization turned on (percent adds every month)
          if (delta === 0) { // delta = 0
            ...
            if (monthSavings[0] === 0) { // Savings = 0
              ...
              $({value: 91}).animate({value: 155}, {
                duration: animatetime, easing: 'swing', step: function () {
          ...
          for (var i = 1; i < 13; i++) {
            depositMonthSavings[i] = ( deltaDeposit
            + parseInt(depositMonthSavings[(i - 1)], 10)
            + ( parseInt(depositMonthSavings[(i - 1)], 10) * percent * 30.41666667 / 36500 ) ) // including delta in
                                                                                               // the last MONTH!
          ...
          monthSavings[12] = (monthSavings[0] * (1 + (savings.percent / 100)) ) + (deltaDeposit * 12) + (deltaNoDeposit
              * 12) + deltapercents; // including delta in the last month!
        }
    ...
    $("#after-savings-value").data({
      "lastvalue": aftervalue,
      "lastmiddlevalue": monthSavings[6]
    }).html(separateNumber(Math.round((savings.freeMoney * 10.222) / 100) * 100));
    ...
    $({value: beforevalue}).animate({value: aftervalue}, {
      duration: animatetime, easing: 'linear', step: function () {
        ...
        $("#month12").children(".large-month-val").children(".val").html(separateNumber(Math.round(this.value / 10)
            * 10));
      }
    });
    ...
    setTimeout(function () {
      ...
      $("#month12").children(".large-month-val").children(".val").html(separateNumber(Math.round(aftervalue / 10)
          * 10));
    }, animatetime + 20);
  ...
  switch (user.checkedCurr) {
    case "RUB":
      $(".curr").html(" Rub ").data("curr", " rub.");
      $(".savings-circle-currency").html(" Rubles ");
      $("#rubcurr").addClass("currchecked");
      break;
    case "EUR":
      $(".curr, .savings-circle-currency").html(" Eur ").data("curr", " \u20ac");
      $("#eurcurr").addClass("currchecked");
      break;
    case "USD":
      $(".curr, .savings-circle-currency").html(" USD ").data("curr", " $");
      $("#usdcurr").addClass("currchecked");
      break;
  }
  ...
  if (incomesSumMonth > expensesSumMonth) {
    setTimeout(function () {
      ...
      $("#incomes-cursor").hide();
      $("#expense-cursor").show();
      animatecircle.call($("#outer-circle"), 0, Math.round(expensesSumMonth), Math.round(incomesSumMonth));
    }, 350);
  } else {
    setTimeout(function () {
      ...
      $("#expense-cursor").hide();
      $("#incomes-cursor").show();
      animatecircle.call($("#outer-circle"), 0, Math.round(incomesSumMonth), Math.round(expensesSumMonth));
    }, 350);
  }
}
...
function animatecircle(beforevalue, aftervalue, sum, title) {
  var before = (100 * beforevalue / sum), after = (100 * aftervalue / sum), R = $(this).data("width") / 2,
      alpha = (after * 0.02 * Math.PI),	// percent to radians
      n = 10,								// margin from circle
      id = $(this).attr("id"), $this = $(this), ycord, xcord;
...
$(".currunchecked").click(function () {
  ...
  initStatisticPage();
  setTimeout(function () {
    initSavingsCircles($("#savings-slider").data("checkedPercent"), 0.2, savings.freeMoney, savings.freeMoney, 700)
  }, 100);
  ...
  $('#savings-slider').noUiSlider({
    ...
    range: {
      'min': [0], 'max': [Math.abs(incomesSumMonth - expensesSumMonth)]
    }
  }, true);
});
...
$("#outermaindiv, #incomes-cursor, #expense-cursor, .lines-title").click(function () {
  setTimeout(function () {
    $("#expenses-lines-container, #incomes-lines-container").toggle();
  }, 250);
...
$("#circle-select-1, #circle-select-2, #circle-select-3").on("change", function () {
  var column, item, circle = this.id, whichCircle;
  switch (circle) {
    case "circle-select-1":
      whichCircle = "first";
      break;
    case "circle-select-2":
      whichCircle = "second";
      break;
    default:
      whichCircle = "third";
  ...
  if ($(this).val() < 100) {
    ...
    item = $(this).val();
    setTimeout(function () {
      initCircle(whichCircle, column[item], 0)
    }, 300);
  } else {
    column = expenses;
    item = $(this).val() - 100;
...
function initSavingsSlider() {
  ...
  $('#savings-slider').noUiSlider({
    ...
    step: (incomesSumMonth - expensesSumMonth) / 20,
    range: {
      'min': [0], 'max': [incomesSumMonth - expensesSumMonth]
    },
    serialization: {
      lower: [new Link({
        target: function (value) {
          $("#savings-slider").data({
            "checkedPercent": Math.round(value
                / (incomesSumMonth - expensesSumMonth + 0.001)
                * 100) / 100
          });
          $(".noUi-handle").attr({
            percent: Math.round(value / (incomesSumMonth - expensesSumMonth + 0.001) * 100)
            + "%"
          })
        }, format: {decimals: 0}
      }), new Link({
        target: function (value) {
          $(".noUi-handle").attr({value: simpleSeparateNumber(Math.round(value / 10) * 10) + $(".curr").data("curr")})
        }, format: {decimals: 0}
      })]
    }
  });
...
$('#savings-slider').on('slide', function () {
  var lastVal = $("#after-savings-value").data("lastvalue"), animatetime;
  if (lastVal < 99999) animatetime = 200; else animatetime = 700;
...
(function () {
  var canvas1 = document.getElementById("movingcircle-1"), canvas2 = document.getElementById("movingcircle-2"),
      canvas3 = document.getElementById("movingcircle-3"), ctx1 = canvas1.getContext('2d'),
      ctx2 = canvas2.getContext('2d'), ctx3 = canvas3.getContext('2d'), x = canvas1.width / 2, y = canvas1.height / 2,
      radius = 99, startAngle = 1 * Math.PI, endAngle = 2.2 * Math.PI, counterClockwise = false;
...
(function () {
  var canvas = document.getElementById("horizontal"), currentDate = new Date(), currentMonth = currentDate.getMonth(),
...
function drawChartLine(position) {
  ...
  var chartWidth = $("#savingschart").width(), chartHeight = $("#savingschart").height(),
  ...
  for (i = 0, j = 12; i < 12; i++, j--) {
    $("#month" + j).css({
      "height": chartHeight - position - i * 7 - ((i * segmentWidth * (chartHeight
      - 153
      - position) ) / chartWidth)
    });
  }
